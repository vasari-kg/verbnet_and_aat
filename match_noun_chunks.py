...
for input in data:
    ...
    for num, sm_sentence in enumerate(sentence_lst):
        ...
        for nc in doc.noun_chunks:
            if num==0:
                phrases.add((nc.text, nc.start_char, nc.end_char))
                phrases.add((doc[nc.root.left_edge.i:nc.root.right_edge.i+1].text,\
                doc[nc.root.left_edge.i:nc.root.right_edge.i+1].start_char, \
                doc[nc.root.left_edge.i:nc.root.right_edge.i+1].end_char))
            else:
                phrases.add((nc.text, nc.start_char+len(sentence_lst[num-1]), nc.end_char+len(sentence_lst[num-1])))
                phrases.add((doc[nc.root.left_edge.i:nc.root.right_edge.i+1].text,\
                doc[nc.root.left_edge.i:nc.root.right_edge.i+1].start_char+len(sentence_lst[num-1]), \
                doc[nc.root.left_edge.i:nc.root.right_edge.i+1].end_char+len(sentence_lst[num-1])))
                            ...
                            if len(set(range(noun_phrase[1],noun_phrase[2])).intersection(range(start_pos,end_pos)))>0:
                                ...
                                for row in visual_works:
                                    ...
                                    if label.startswith("<"):
                                        ...
                                        if string_1 == string_2:
                                            output.append((int(input["id"]), noun_phrase[1], noun_phrase[2], noun_phrase[0], aat_id, label, "visual_work"))
                                for row in buildings:
                                    ...
                                    if label.startswith("<"):
                                        ...
                                        string_2 = " ".join([ps.stem(w) for w in label_no_rounds.split(" ")])
                                        if string_1 == string_2:
                                            output.append((int(input["id"]), noun_phrase[1], noun_phrase[2], noun_phrase[0], aat_id, label, "building"))
        except KeyError:
...
with open('matched_terms_final', 'w', encoding="utf-8") as f:
    write = csv.writer(f)
